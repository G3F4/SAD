...
Texture::Texture(std::string ID, std::string file, int x, int y, int width, int height, double angle, SDL_Point *pivot_point, SDL_RendererFlip flip, bool state):
                BaseObject(ID,x,y,width,height,state), m_file(file), m_angle(angle), m_pivot_point(pivot_point), m_flip(flip)
{
